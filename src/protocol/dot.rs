...
impl DNSOverTLS {
  pub async fn new(
    ...
    let addrs: Vec<SocketAddr> = {
      ...
      if let Ok(addr) = upstream.parse() {
        ...
        if use_hosts {
          ...
          let port: u16 = if let Some(p) = y.pop() {
            p.parse()?
          } else {
          ...
          if let Some(ips) = HOSTS.lookup(&host).await {
            for ip in ips.iter() {
              x.push(SocketAddr::new(*ip, port));
            }
          } else {
    ...
    for addr in addrs.iter() {
      sessions.insert(*addr, VecDeque::new()).unwrap();
    }

    // start connection keep-alive watchdog
    let _task = smolscale2::spawn(Self::_conn_watchdog(
      connector.clone(),
      sessions.clone(),
    ));
  ...
  async fn _conn_watchdog(
    ...
    loop {
      while let Some(addr) = reconnecting.pop() {
        // connecting...
        let tcp_conn = match TcpStream::connect(addr)
          .await
        {
          Ok(v) => v,
          Err(err) => {
        ...
        match connector
          .connect(addr.ip().to_string(), tcp_conn)
          .await
        {
          Ok(tls_conn) => {
            let id = {
              let rand = fastrand::u64(..);
              let ptr = core::ptr::addr_of!(tls_conn);
            ...
            {
              entry.get_mut().push_back((id, tls_conn));
            }
          },
      ...
      for (addr, io) in ret.iter_mut() {
        ...
        for i in 0..io.len() {
          let (id, io) = &io[i];
          if let Err(err) = io.peek(&mut [0]).await {
            log::warn!("session died: addr={addr:?} | io={io:?} | error={err:?}");
            reconnecting.push(*addr);

            // remove dead conn
            sessions
              .update_async(addr, |_, conns| {
