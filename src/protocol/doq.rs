...
impl DNSOverQUIC {
  pub fn new(
    ...
    let transport_config = {
      ...
      tc.initial_mtu(1200);
      tc.keep_alive_interval(Some(Duration::from_secs(
        10,
      )));
    ...
    let udp_socket =
      std::net::UdpSocket::bind(if addr.is_ipv4() {
        "0.0.0.0:0"
      } else {
        "[::]:0"
      })?;
    ...
    let _task =
      Arc::new(smolscale2::spawn(Self::_conn_task(
        endpoint.clone(),
        conns.clone(),
        addr,
      )));
  ...
  async fn _conn_task(
    ...
    loop {
      ...
      if let Ok(conn) = connecting
        .await
        .context("unable connect to remote DoQ server")
        .log_warn()
      {
        let ptr = core::ptr::addr_of!(conn) as u128;

        let conn = Arc::new(conn);

        let mut id: u128 = 0;
        while let Err(_) =
          conns.insert_async(id, conn.clone()).await
        {
