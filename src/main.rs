...
impl DNSDaemon {
  pub async fn new(
    ...
    if let Some(ref hosts_filename) = opt.hosts {
      let _ =
        HOSTS.load(hosts_filename).await.log_error();
    }
    ...
    let resolvers = {
      ...
      for dot_addr in opt.dot_upstream.iter() {
        x.push(Arc::new(
          DNSOverTLS::new(dot_addr, opt.hosts.is_some())
            .await?,
        ));
      }
...
static ENV_FILTER: Lazy<Option<env_filter::Filter>> =
  Lazy::new(|| {
    if let Ok(ref env) = std::env::var("RUST_LOG") {
      Some(env_filter::Builder::new().parse(env).build())
    } else {
      None
    }
  });
    ...
    impl ftlog2::FtLogFormat for MyFmt {
      ...
      fn msg(
        ...
        let level_style = match level {
          ...
          Error => {
            Red.on_default().effects(Effects::BOLD)
          },
        };
        let level = format!(
          "{}{}{}",
        ...
        let args = {
          ...
          a.as_str()
            ...
            .unwrap_or_else(|| Cow::Owned(a.to_string()))
        };
