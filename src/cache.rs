...
pub struct DNSCacheEntry {
  ...
  update_task: Arc<
    RwLock<Option<Arc<smol::Task<anyhow::Result<()>>>>>,
  >,
      ...
      *self.update_notify.write().await = Some(update_rx);
      ...
      let task = smolscale2::spawn(async move {
        ...
        let expire = SystemTime::now()
          + Duration::from_secs(expire_ttl as u64);
        ...
        *entry_lock.write().await = Some(entry.clone());
        ...
        let _ = update_tx.send(()).await.log_error();
...
impl DNSCache {
  ...
  pub(crate) async fn load(&self) -> anyhow::Result<()> {
    ...
    {
      let mut ret =
        sqlx::query("SELECT * FROM hitdns_cache_v1")
          .fetch(&*HITDNS_SQLITE_POOL);
      while let Ok(Some(line)) = ret.try_next().await {
        assert_eq!(line.columns().len(), 2);
        let query: Vec<u8> =
          line.try_get_raw(0)?.to_owned().decode();
        let entry: Vec<u8> =
          line.try_get_raw(1)?.to_owned().decode();
